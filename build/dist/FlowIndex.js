/* src/FlowIndex.svelte generated by Svelte v3.31.2 */
import {
	SvelteComponent,
	append,
	attr,
...
import FlowStub from "./FlowStub.js";
...
// (33:12) {#each (Object.entries($flows) || []) as flow}
function create_each_block(ctx) {
	let flowstub;
	let current;

	flowstub = new FlowStub({
			props: {
				flow: /*flow*/ ctx[2],
				router: /*router*/ ctx[0]
			}
	...
	return {
		c() {
			create_component(flowstub.$$.fragment);
		},
		m(target, anchor) {
			mount_component(flowstub, target, anchor);
			current = true;
		},
		p(ctx, dirty) {
			const flowstub_changes = {};
			if (dirty & /*$flows*/ 2) flowstub_changes.flow = /*flow*/ ctx[2];
			if (dirty & /*router*/ 1) flowstub_changes.router = /*router*/ ctx[0];
			flowstub.$set(flowstub_changes);
		},
		i(local) {
			if (current) return;
			transition_in(flowstub.$$.fragment, local);
			current = true;
		},
		o(local) {
			transition_out(flowstub.$$.fragment, local);
			current = false;
		},
		d(detaching) {
			destroy_component(flowstub, detaching);
		}
	};
}
...
function create_fragment(ctx) {
	...
	let tbody;
	let current;
	let each_value = Object.entries(/*$flows*/ ctx[1]) || [];
	let each_blocks = [];
	...
	return {
		c() {
			...
			thead.innerHTML = `<tr><th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Flow</th> 
		...
		p(ctx, [dirty]) {
			if (dirty & /*Object, $flows, router*/ 3) {
				each_value = Object.entries(/*$flows*/ ctx[1]) || [];
				let i;
...
function instance($$self, $$props, $$invalidate) {
	let $flows;
	component_subscribe($$self, flows, $$value => $$invalidate(1, $flows = $$value));
	let { router } = $$props;
...
class FlowIndex extends SvelteComponent {
	constructor(options) {
		super();
